...
- 安裝、更新與移除改為可隱藏 Winget 視窗的背景作業，並保留顯示視窗選項
- 單一與批次操作在底部顯示整體進度、完成數及失敗資訊
- 工作執行期間自動鎖定相關按鈕與右鍵功能，完成或失敗後恢復使用
- 更新、安裝與移除完成後自動重新掃描相關清單
- 改善非標準套件識別碼的移除流程：識別碼失敗時會改用程式名稱重試
- 錯誤狀態可手動清除，並改善背景作業回饋與操作穩定性
