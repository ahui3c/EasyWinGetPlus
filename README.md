...
- 修正程式由隱藏的 Windows PowerShell 承載時，工作列錯誤顯示 PowerShell 圖示
- 主視窗、首次語言選擇與錯誤報告視窗現在會從 EasyWinGetPlus.exe 載入正式圖示
...
- 修正繁中或其他本地化 Windows 環境中，Winget 輸出編碼／欄名差異可能造成所有掃描結果皆為零筆
- Winget 輸出改以 UTF-8 讀取，並加入不依賴語言欄名的欄位順序備援解析
- 無法解析資料時會明確顯示錯誤，不再誤判為零筆成功
- 自動建立每日運行診斷 Log；可從「設定 → 運行診斷紀錄」直接開啟資料夾並回傳最新檔案
- Release 同時提供可攜版與完整安裝版；安裝版包含開始功能表、選用桌面捷徑及標準解除安裝功能
